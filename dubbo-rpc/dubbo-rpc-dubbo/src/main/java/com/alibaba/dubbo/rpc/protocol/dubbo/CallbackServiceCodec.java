...
/**
 * callback service helper 该类是针对回调服务的编解码器。
 */
class CallbackServiceCodec {
    ...
    private static String exportOrunexportCallbackService(Channel channel, URL url, Class clazz, Object inst, Boolean export) throws IOException {
        ...
        // no need to new client again  // 设置不是服务端标志为否
        params.put(Constants.IS_SERVER_KEY, Boolean.FALSE.toString());
        // mark it's a callback, for troubleshooting  // 设置是回调服务标志为true
        ...
        if (export) {
            ...
            if (!channel.hasAttribute(cacheKey)) {
                if (!isInstancesOverLimit(channel, url, clazz.getName(), instid, false)) {
                    ...
                    // should destroy resource? // 暴露服务
                    Exporter<?> exporter = protocol.export(invoker);
                    // this is used for tracing if instid has published service or not.  // 放到通道
                    channel.setAttribute(cacheKey, exporter);
                    logger.info("export a callback service :" + exporturl + ", on " + channel + ", url is: " + url);
                    increaseInstanceCount(channel, countkey);// 计数器加1
                }
            }
        } else {
            // 如果通道内已经有该服务的缓存
        ...
        if (isRefer) {// 如果是服务引用
            ...
            if (proxy != null) {
                ...
                try {
                    ...
                    invoker.destroy(); // 销毁该调用
                } catch (Exception e) {
                    logger.error(e.getMessage(), e);
                }
                // cancel refer, directly remove from the map  // 取消引用，直接从集合中移除
                channel.removeAttribute(proxyCacheKey);
                channel.removeAttribute(invokerCacheKey);
                decreaseInstanceCount(channel, countkey);// 计数器减1
            }
        }
        return proxy;
    }
