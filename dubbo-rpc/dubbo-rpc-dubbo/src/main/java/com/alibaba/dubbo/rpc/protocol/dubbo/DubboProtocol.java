...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        exporterMap.put(key, exporter); // 加入到集合
    ...
    private ExchangeServer createServer(URL url) {
        ...
        // enable heartbeat by default  // 心跳默认间隔一分钟
    ...
    private ExchangeClient[] getClients(URL url) {
        // whether to share connection // 一个连接是否对于一个服务
    ...
    /**
     * Get shared connection  该方法是获得分享的客户端连接。
     */
    private ExchangeClient getSharedClient(URL url) {
        ...
        synchronized (locks.get(key)) {
            ...
            locks.remove(key);// 从对象锁中移除
            return client;
        }
    }

    /**
     * Create new connection  该方法是新建一个客户端连接
     */
    private ExchangeClient initClient(URL url) {

        // client type setting.  // 获得客户端的实现方法 默认netty3
        ...
        try {
            // connection should be lazy  // 是否需要延迟连接，，默认不开启
