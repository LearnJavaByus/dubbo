 ...
 * AbstractServer
 *
 * 实现了服务器的公共逻辑，比如发送消息，关闭通道，连接通道，断开连接等。并且抽象了打开和关闭服务器两个方法。
 */
public abstract class AbstractServer extends AbstractEndpoint implements Server {
    ...
    /**
     * 大可连接的客户端数量、空闲超时时间以及线程池的两个配置参数。其中要注意核心线程数和最大线程数的区别。
     * 举个例子，核心线程数就像是工厂正式工，最大线程数，就是工厂临时工作量加大，请了一批临时工，临时工加正式工的和就是最大线程数，等这批任务结束后，
     * 临时工要辞退的，而正式工会留下。
     *
     *
     *
