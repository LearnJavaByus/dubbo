...
/**
 * AbstractClusterInvoker  该类实现了Invoker接口，是集群Invoker的抽象类。
 *
 */
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
     ...
     *
     * 该方法实现了使用负载均衡策略选择一个调用者。首先，使用loadbalance选择一个调用者。如果此调用者位于先前选择的列表中，
     * 或者如果此调用者不可用，则重新选择，否则返回第一个选定的调用者。重新选择，重选的验证规则：选择>可用。
     * 这条规则可以保证所选的调用者最少有机会成为之前选择的列表中的一个，也是保证这个调用程序可用。
     *
     *
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation, List<Invoker<T>> invokers, List<Invoker<T>> selected) throws RpcException {
        ...
        {
            //ignore overloaded method   // 如果上一次粘滞连接的调用不在可选的提供者列合内，则直接设置为空
        ...
        if ((selected != null && selected.contains(invoker))
            ...
            try {
                ...
                if (rinvoker != null) {
                    ...
                    try {
                        //Avoid collision  // 最后再避免选择到同一个invoker
    ...
    public Result invoke(final Invocation invocation) throws RpcException {
        ...
        // binding attachments into invocation.    // 获得上下文的附加值
