...
public class DubboRegistry extends FailbackRegistry {
    ...
    // Reconnecting detection cycle: 3 seconds (unit:millisecond) // 重新连接周期：3秒
    private static final int RECONNECT_PERIOD_DEFAULT = 3 * 1000;

    // Scheduled executor service // 任务调度器
    ...
    /**
     * The time in milliseconds the reconnectTimer will wait // 任务调度器reconnectTimer将等待的时间
     */
    ...
    public DubboRegistry(Invoker<RegistryService> registryInvoker, RegistryService registryService) {
        ...
        // Start reconnection timer  // 优先取url中key为reconnect.perio的配置，如果没有，则使用默认的3s
    ...
    protected final void connect() {
        try {
            // Check whether or not it is connected // 检查注册中心是否已连接
            if (isAvailable()) {
                return;
            }
            ...
            clientLock.lock(); // 获得客户端锁
            try {
                // Double check whether or not it is connected  // 二次查询注册中心是否已经连接
                if (isAvailable()) {
                    return;
                }
                recover(); // 恢复注册和订阅
    ...
    /**
     * 销毁重连计时器、注册中心的Invoker和任务调度器，
     */
    @Override
    public void destroy() {
        super.destroy();
        try {
            // Cancel the reconnection timer // 取消重新连接计时器
        ...
        registryInvoker.destroy(); // 销毁注册中心的Invoker
        // 关闭任务调度器
